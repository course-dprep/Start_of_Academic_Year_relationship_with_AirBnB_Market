...
For this project we used publicly available archival data from Airbnb. In total, ten datasets were retrieved from the website www.insideairbnb.com, two for each city of interest. For each city of the five cities two datasets were downloaded "Listings.csv" and "calendar.csv" both containing different variables. The listings dataset contained information per listing about the type of property and it's features. The calendar dataset contained info on the price of the property on different dates and it's availability. We combined these datasets together on id to create one dataset for each city. This data was then explored (see: "Data exploration raw data"). Later a selection of the variables we were interested in was made as well as selecting only specific dates. This is described in more detail under "Data Transformation". 

##### Data exploration raw data 
First the raw data was explored. For this we looked at the variable of interest to our research question. For the metric variables we made boxplots and took the summary statistics. For the categorical variable we took the count of each one to get a insight into their comperative numbers. 
...
In order to answer our research questions, we used linear regression models. A linear regression is used to predict the effects of one or more predictors on one dependent variable. Our aim was to investigate the relationship between a nonmetric independent variable on a metric dependent variable. To examine this, a linear regression was the most obvious analysis to do. Some other options were discussed, such as an ANOVA, a repeated-measures ANOVA and the MANOVA, but a regression analysis seemed both the most feasible as the best answering method of analysis. In addition, interaction effects can be measured as well in a linear regression. We wanted to examine if there are any moderation effects for host type and room type, which can be measured by linear regression. This made it even more clear we wanted to use regression as our statistical analysis.

Per city (Amsterdam, Antwerp, Berlin, Brussels and Rotterdam), we performed one regression with the independent variable ('dummy_month_aug') on the price ('average_price') and one regression with the independent variable ('dummy_month_aug') on the maximum nights a listing was available ('average_nights'). This means that in total, we performed ten regression analyses.
...
First regression on price 

For Antwerp There is no significant effect observed for the start of the academic year on price. Room type and superhost do appear to have a significant effect on the price. With entire apartments (value 1) the price goes up €28.43 compared to the other room types. When the host is a superhost the price actually goes up by €6.25. The interactions between the independent variables are insignificant. 

For Amsterdam there is no significant effect observed for the start of the academic year on price. The room type is a very significant with a p-value below 0.001 and shows a increase of €54.22 when the room type is entire apartment.  Host_is_superhost  is also significant causing a €8.11 decrease and the interactions are not significant. 

For Berlin there is no significant effect observed for the start of the academic year on price. The p-value is very high with 0.26. The only significant moderators are room type which has a very low p-value bellow 0.01 and increases the price by €37.74 and host_is_superhost increasing the price by €14.34.  

For Brussels there is no significant effect observed for the start of the academic year on price. The room type is a very significant with a p-value below 0.001 and shows a increase of €26.18 when the room type is entire apartment.  Host_is_superhost  is also significant causing a €3.74 increase and the interactions are not significant. 

For Rotterdam there is no significant effect observed for the start of the academic year on price. The room type is significant causing a €39.48 increase. And the host_is_superhost IV is also significant and causes a €8.47 decrease in price. 


```{r}
```
Second regression on average nights:

For Antwerp there is no significant effect observed for the start of the academic year on average nights. Only the moderator host_is_superhost is significant causing a 131.14 increase.

For Amsterdam there is no significant effect observed for the start of the academic year on average nights. Only the moderator host_is_superhost is significant causing a 152.34 increase and room type causes a 135.13 decrease. 

For Berlin there is no significant effect observed for the start of the academic year on average nights. The moderator room type causes a significant 47.40 increase and as significant 88.54 increase for host_is_superhost. 

For Brussels there is no significant effect observed for the start of the academic year on average nights. Here room type has a significant effect, causing an increase of 80.45 and host_is_superhost is causing a significant 71.11 increase. 

For Rotterdam there is no significant effect observed for the start of the academic year on average nights. None of the moderators are significant.
