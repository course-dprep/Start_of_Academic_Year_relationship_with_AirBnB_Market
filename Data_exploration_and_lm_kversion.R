...
clean_cal_price_aug <- function (dataset) {
  ...
  dataset_wide_clean$average_price_aug  <- round(dataset_wide_clean$average_price, digits = 2)
...
clean_cal_nights_aug <- function(dataset){
  ...
  # Select only useful columns from calendar data set
  dataset_wide_clean <- dataset_wide_clean %>% select(id, average_nights_aug)
...
clean_cal_nights_mar <- function(dataset){
  ...
  # Take the average price per listing from August/September and March/April
  dataset_wide_clean <- dataset_wide_clean %>% 
    mutate(average_nights_mar = rowMeans(dataset_wide_clean[,2:33]))
  
  dataset_wide_clean$average_nights  <- round(dataset_wide_clean$average_nights_mar, digits = 2)
  # Rename listing_id to id so it can be used to merge 
  dataset_wide_clean <- dataset_wide_clean %>%  rename(id = listing_id)
  
  # Select only useful columns from calendar data set
  dataset_wide_clean <- dataset_wide_clean %>% select(id, average_nights_mar)
...
lm2<- lm(average_price ~ room_type + host_is_superhost + dummy_month_aug, total_antwerp)
